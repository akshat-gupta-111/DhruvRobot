...
if __name__ == "__main__":
    ...
    print("[TEST] Moondream Cloud API -- single-frame test")

    # 1. Grab one frame from webcam
    cap = cv2.VideoCapture(0)
    if not cap.isOpened():
        print("[ERROR] Camera not accessible."); exit(1)
    ...
    if not ok:
        print("[ERROR] Failed to read frame."); exit(1)

    print("[OK] Frame captured -- sending to Moondream API...")
